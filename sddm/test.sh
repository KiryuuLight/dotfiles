...
if [ ! -d "/usr/share/sddm/themes/catppuccin-mocha" ]; then
    ...
    echo -e "Run ${cyan}'./install.sh'${reset} or copy the contents of the theme to ${cyan}'/usr/share/sddm/themes/catppuccin-mocha/'${reset} and set the current theme to ${cyan}'catppuccin-mocha'${reset} in ${cyan}'/etc/sddm.conf'${reset}:\n"
    echo -e "    ${grey}# /etc/sddm.conf${reset}"
    echo -e "    [Theme]"
    echo -e "    Current=catppuccin-mocha"
fi
